...
for layer in range(model.cfg.n_layers):
    names_filter.append(f'blocks.{layer}.hook_y')
    ...
    def generate_replacement_hook(layer, patch_layer, cache, patch_cache):
        if layer == patch_layer:
            def replacement_hook(activations: torch.Tensor, hook: Any):
                activations[:, -1] = patch_cache[f'blocks.{layer}.hook_y'][:, -1]
                return activations
        else:
            def replacement_hook(activations: torch.Tensor, hook: Any):
                activations[:, -1] = cache[f'blocks.{layer}.hook_y'][:, -1]
                return activations
...
result_dir = 'circuit/induction/mamba/result/y_to_logit'
