...
sum_logit_new = torch.zeros([up_layer, model.cfg.n_heads]).to(device)
sum_p_new = torch.zeros([up_layer, model.cfg.n_heads]).to(device)
sum_logit_orig = torch.zeros([up_layer, model.cfg.n_heads]).to(device)
sum_p_orig = torch.zeros([up_layer, model.cfg.n_heads]).to(device)
...
for batch in tqdm(range(N), desc='Batch'):
    ...
    def generate_head_out_replacement_hook(layer, position, patch_layer, patch_head, cache, patch_cache):
        if layer == patch_layer:
            def replacement_hook(activations: torch.Tensor, hook: Any):
                activations[torch.arange(batch_size), position] = cache[f'blocks.{layer}.attn.hook_z'][torch.arange(batch_size), position]
                activations[torch.arange(batch_size), position, patch_head] = patch_cache[f'blocks.{layer}.attn.hook_z'][torch.arange(batch_size), position, patch_head]
                return activations
        else:
            def replacement_hook(activations: torch.Tensor, hook: Any):
                activations[torch.arange(batch_size), position] = cache[f'blocks.{layer}.attn.hook_z'][torch.arange(batch_size), position]
    ...
    for layer in range(up_layer):
        for head in range(model.cfg.n_heads):
            fwd_hooks = []
            for l in range(model.cfg.n_layers):
                hook_name = f'blocks.{l}.attn.hook_z'
                fwd_hooks.append((hook_name, generate_head_out_replacement_hook(l, source_index + 1, layer, head, cache, patch_cache)))
            resid = model.run_with_hooks(induction_input, fwd_hooks=fwd_hooks, stop_at_layer=up_layer)
            _, head_cache = model.run_with_cache(resid, start_at_layer=up_layer, stop_at_layer=up_layer + 1)
            fwd_hooks = []
            for l in range(model.cfg.n_layers):
                hook_name = f'blocks.{l}.attn.hook_z'
                fwd_hooks.append((hook_name, generate_head_out_replacement_hook(l, object_index, up_layer, up_head, cache, head_cache)))
...
result_dir = 'circuit/induction/pythia/result/head_to_L5.0_to_logit'
