...
model = HookedMamba.from_pretrained('path/to/model', tokenizer=tokenizer, device=device)
...
start_layer_num = 0
end_layer_num = 24
num_layers = end_layer_num - start_layer_num
...
with tqdm(total=total_tokens, desc='Sampled tokens') as pbar:
    while current_tokens < total_tokens:
        ...
        # 计算激活值
        _, cache = model.run_with_cache(tokens, names_filter=[f'blocks.{l}.hook_ssm_input' for l in range(start_layer_num, end_layer_num)] + [f'blocks.{l}.hook_skip' for l in range(start_layer_num, end_layer_num)], fast_conv=True, fast_ssm=True)

        # 保存激活
        for l in range(start_layer_num, end_layer_num):
            # 获取过滤后的激活值

            mamba_ssm_input = cache[f'blocks.{l}.hook_ssm_input'][filter_mask].to(torch.bfloat16)
            mamba_silu_skip = F.silu(cache[f'blocks.{l}.hook_skip'])[filter_mask].to(torch.bfloat16)

            activations = torch.cat((mamba_ssm_input, mamba_silu_skip), dim=1)
