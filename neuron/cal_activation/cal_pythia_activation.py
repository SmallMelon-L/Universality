...
with tqdm(total=total_tokens, desc='Sampled tokens') as pbar:
    while current_tokens < total_tokens:
        ...
        # 计算激活值
        _, cache = model.run_with_cache(tokens, names_filter=[f'blocks.{l}.mlp.hook_post' for l in range(start_layer_num, end_layer_num)])

        # 保存激活
        for l in range(start_layer_num, end_layer_num):
            # 获取过滤后的激活值
            activations = cache[f'blocks.{l}.mlp.hook_post'][filter_mask].to(torch.bfloat16)
