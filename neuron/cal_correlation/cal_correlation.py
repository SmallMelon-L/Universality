...
# 获取命令行输入的参数
if len(sys.argv) != 7:
    print("Usage: python script_name.py <layer_1> <layer_2> <d_model> <exp_factor> <model_name_1> <model_name_2>")
    sys.exit(1)
...
m = torch.zeros([d_model * exp_factor], dtype=torch.float64).to(device)
p = torch.zeros([d_model * exp_factor], dtype=torch.float64).to(device)
mm = torch.zeros([d_model * exp_factor], dtype=torch.float64).to(device)
pp = torch.zeros([d_model * exp_factor], dtype=torch.float64).to(device)
mp = torch.zeros([d_model * exp_factor, d_model * exp_factor], dtype=torch.float64).to(device)
...
with tqdm(total=total_tokens, desc=f'sampled tokens') as pbar:
    while current_tokens < total_tokens:
        ...
        features_1 = act_1.to(torch.float64)
        features_2 = act_2.to(torch.float64)
...
del act_1, act_2, features_1, features_2
...
correlation_matrix = torch.zeros([d_model * exp_factor, d_model * exp_factor], dtype=torch.float64).to(device)
...
for i in range(num_blocks):
    ...
    for j in range(num_blocks):
        ...
        denominator = torch.sqrt(
            torch.clamp((n * mm[start_i:end_i] - m_block ** 2).unsqueeze(1) @
            (n * pp[start_j:end_j] - p_block ** 2).unsqueeze(0), min=1e-8)
        )
        denominator = torch.clamp(denominator, min=1e-16)
