...
# 激活保存路径
os.makedirs('path/to/output_dir', exist_ok=True)

# 初始化层激活保存的文件夹
start_layer_num = 0
end_layer_num = 24
num_layers = end_layer_num - start_layer_num
layer_dirs = [os.path.join('path/to/output_dir', f'layer_{l}') for l in range(start_layer_num, end_layer_num)]
...
with tqdm(total=total_tokens, desc='Sampled tokens') as pbar:
    while current_tokens < total_tokens:
        ...
        # 计算激活值
        _, cache = model.run_with_cache(tokens, names_filter=[f'blocks.{l}.hook_resid_post' for l in range(start_layer_num, end_layer_num)], fast_conv=True, fast_ssm=True)
