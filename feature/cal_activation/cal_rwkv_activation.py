...
with tqdm(total=total_tokens, desc='Sampled tokens') as pbar:
    while current_tokens < total_tokens:
        ...
        tokens = tokenizer(batch["text"], return_tensors="pt", padding=True, truncation=True, max_length=max_length).input_ids.to(device)
        ...
        # 计算激活值
        return_dict = model(tokens, output_hidden_states=True)
        hidden_states = return_dict.hidden_states
        
        # 保存激活
        for l in range(start_layer_num, end_layer_num):
            # 获取过滤后的激活值
            activations = hidden_states[l][filter_mask].to(torch.bfloat16)
